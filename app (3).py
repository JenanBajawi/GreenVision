...
# ============================================================
HF_REPO_ID = "JenanB/greenvision-model"
MODEL_FILES = ["plant_model.pth", "severity_model.pth"]
...
def download_models():
    ...
    Returns a dict mapping filename -> local path of the downloaded file.
    """
    ...
    for filename in MODEL_FILES:
        try:
            ...
            st.error(f"❌ Failed to download **{filename}** from `{HF_REPO_ID}`.\n\n```\n{e}\n```")
            paths[filename] = None
    return paths


# Trigger download before model loading
...
def load_classification_model():
    ...
    path = MODEL_PATHS.get("plant_model.pth")
...
def load_regression_model():
    ...
    path = MODEL_PATHS.get("severity_model.pth")
    model = models.resnet34(pretrained=False)
