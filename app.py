...
classifier_model = models.resnet34(weights=None)
...
severity_model.fc = torch.nn.Sequential(
    torch.nn.Linear(severity_model.fc.in_features, 128),
    torch.nn.ReLU(),
...
def render_probability_bar(name, value, is_top=False):
    ...
    <div class="prob-row">
        <div class="prob-label">
            ...
            <span class="prob-value">{value*100:.1f}%</span>
        </div>
        <div class="prob-bar-bg">
            <div class="{bar_class}" style="width: {value*100}%;"></div>
        </div>
    </div>
    """
...
if uploaded_file is not None:
    ...
    for i, rec in enumerate(recommendations, 1):
        ...
        <div class="recommendation-item">
            <div class="recommendation-number">{i:02d}</div>
...
# ============================================================
    ...
    for i, (key, info) in enumerate(DISEASE_INFO.items(), 1):
        cards_html += f"""
        <div class="info-card fade-up delay-{min(i, 4)}">
            <div class="info-card-number">№ {i:02d} · {info['icon']}</div>
            <div class="info-card-title">{info['common_name']}</div>
            <div class="info-card-latin">{info['latin_name']}</div>
            <div class="info-card-desc">{info['description']}</div>
        </div>
        """
    cards_html += '</div>'
